	...
	function customAutoComplete ( opt ) {
		...
		$(inputSelector).live(eventName,function ( $event ) {
			...
			$("#"+divId).remove();
			var innerDiv = $("<div/>").attr("id",divId).css({"z-index":"500" , "position":"absolute" , "right":"0px" , "width" : width + "px" , "border" : "1px solid black" , "border-top" : "0px" , "display" : "none" ,"max-height": "200px" , "overflow-y" : "auto"});
				...
				success : function ( data ) {
					...
					$(data).each(function(index){
						...
						$(li).bind( "mouseover" , info , function ($event) {
							selectIndex = index;
							selectChange();
							eval(clickMethodName + "(" + $event.data.toSource() + ")");
						});
						
						var showText = eval ( showMethodName + "(" + info.toSource() + ")" );
						$(li).text(showText);
