...
<style>[enableEdit='false']{display:none; visibility:hidden;}</style>
		...
		$(function(){
			...
			$(".container-tab2 ul li").each(function(index){
				$(this).click(function(){
					...
					if ( $(this).text().indexOf("日历显示") >= 0 ) {
						$(".duty_kind").css({"display":"inline"});
						$(".duty_kind2").css({"display":"none"});
					} else {
						$(".duty_kind2").css({"display":"inline"});
						$(".duty_kind").css({"display":"none"});
					}
...
<%-- 页面 js --%>
<script type="text/javascript" src="js/staffduty.js"></script>
