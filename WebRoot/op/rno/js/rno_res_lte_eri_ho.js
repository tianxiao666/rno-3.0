...
function bindEvent(){
	$("#importBtn").click(function() {
		...
		$("#err").remove();
		if ($("#fileid").val() == "") {
			$("#fileid").parent().append("<font id='err' color='red'>请选择爱立信切换文件</font>");
			return;
		}
		if ($.trim($("#meatime").val()) == "") {
 ...
 * 查询爱立信切换描述数据
 */
function queryEriHoDescData(){
	$("#searchNcsForm").ajaxSubmit({
		url:'queryEri4GHoDescAjaxAction',
		type:'post',
		dataType:'text',
		success:function(raw){
			var data={};
			...
			displayEriHoDescData(data['data']);
 ...
 * @param data
 */
function displayEriHoDescData(data){
	if(data==null||data==undefined){
		return;
	}
	//
