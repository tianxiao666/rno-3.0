...
$(document).ready(function() {
	...
	$("#importBtn").click(function() {
		$("#autoload").val("false");
		var area=$("#handover_areaId").find("option:selected").text();
         $("#chareaname").val(area);
...
function initAreaCascadeHandover(){
	// 联动----区域1
	$("#handover_provinceId").change(function() {
		getSubAreas("handover_provinceId", "handover_cityId", "市");
	});
	$("#handover_cityId").change(function() {
		getSubAreas("handover_cityId", "handover_areaId", "区/县");
	});
	
	// 联动----区域2
	$("#handover_provinceId2").change(function() {
		getSubAreas("handover_provinceId2", "handover_cityId2", "市");
	});
	$("#handover_cityId2").change(function() {
		getSubAreas("handover_cityId2", "handover_areaId2", "区/县");
	});
}
...
function queryAndLoadResource() {
	...
	var area=$("#handover_areaId2").find('option:selected').text().trim();
 ...
 * @description 在切换数据搜索窗口选择数据时，刷新切换数据分析列表
 */
function refreshHandoverAnalysisList(obj,tabName){
