...
function bindEvent(){
	$("#importBtn").click(function() {
		...
		if(!(filename.toUpperCase().endsWith(".CSV")||filename.toUpperCase().endsWith(".XLS")
				||filename.toUpperCase().endsWith(".XLSX")||filename.toUpperCase().endsWith(".ZIP"))){
	...
	//查询切换描述数据
	$("#searchDescBtn").click(function(){
		initFormPage("searchDescForm");
		queryDescData();
	});
 ...
 * 查询切换描述数据
 */
function queryDescData(){
	$("#searchDescForm").ajaxSubmit({
		url:'querySfDescAjaxAction',
		type:'post',
		dataType:'text',
		success:function(raw){
			var data={};
			...
			displayDescData(data['data']);
			setFormPageInfo("searchDescForm",data['page']);
			setPageView(data['page'],"listPageDiv");
		}
	});
}
 ...
 * @param data
 */
function displayDescData(data){
	if(data==null||data==undefined){
		return;
	}
	//
	$("#descListTab tr:not(:first)").each(function(i, ele) {
		$(ele).remove();
	});
	var html="";
