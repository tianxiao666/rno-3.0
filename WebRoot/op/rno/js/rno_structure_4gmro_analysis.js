...
$(document).ready(
	...
	$("#addStructureTask").click(function() {
		...
		location.href = 'add4GMroStructureTaskInfoPageForAjaxAction?cityId='+cityId;
	});
...
function g4MroStructureAnalysis() {
	...
	$.ajax({
		url : 'submit4GMroStructureTaskAction',
		data : {
			'lteTaskInfo.provinceId':provinceId2,
			'lteTaskInfo.provinceName':provinceName,
			'lteTaskInfo.cityId':cityId2,
			'lteTaskInfo.cityName':cityName,
			'lteTaskInfo.taskName':taskName,
			'lteTaskInfo.taskDesc':taskDescription,
			'lteTaskInfo.startTime':beginTime,
			'lteTaskInfo.endTime':latestAllowedTime
		},
		type : 'post',
		dataType : 'text',
		success : function(raw) {
			...
			try {
				...
				if (data['flag'] == false) {
					...
					// location.href="init4GMroAnalysisPageAction";
...
function sumbitStructureForm() {
	...
	$("#structureTaskForm").ajaxSubmit({
		url : 'queryLteStructureAnaTaskByPageForAjaxAction',
		dataType : 'text',
		success : function(raw) {
