...
function bindEvent(){
	...
	$("#citymenu").change(function() {	
		...
		clearCellSelect();
		//再填充
		get2GHwCellQueryDateAndBscListInfo(cityId,"cell");
	});
	...
	$("#citymenu2").change(function() {	
		...
		clearChannelSelect();
		//再填充
		get2GHwCellQueryDateAndBscListInfo(cityId,"channel");
	});
	...
	$("#citymenu3").change(function() {	
		...
		clearNcellSelect();
		//再填充
		get2GHwCellQueryDateAndBscListInfo(cityId,"ncell");
	});
	//列表改变事件
	...
	$("#searchCellBtn").click(function(){
		var cell=$("#cell_targetCell").find("option:selected").text();
		if(ifHasSpecChar(cell)){
			$("#cell_CellErr").text("有特殊字符!");
			return;
		}else{
			$("#cell_CellErr").text("");
		}
	...
	/*$("#searchCellChannelBtn").click(function(){
		...
		queryHw2GCellChannelData();
	});*/
	//查询华为小区邻区数据
	$("#searchNcellBtn").click(function(){
		var  cell=$("#ncell_targetCell").find("option:selected").text();
		var ncell=$("#ncell_targetNcell").find("option:selected").text();
		if(ifHasSpecChar(cell)){
			$("#ncell_CellErr").text("有特殊字符!");
			return;
		}else{
			$("#ncell_CellErr").text("");
		}
		if(ifHasSpecChar(ncell)){
			$("#ncell_NcellErr").text("有特殊字符!");
			return;
		}else{
			$("#ncell_NcellErr").text("");
		}
 ...
 * @version 1.2
 */
function queryHw2GCellData(){
	$("#progressDesc").html("");
	$(".loading_fb").text("华为2G小区数据");
	...
	$("#searchCellForm").ajaxSubmit({
		url:'queryHw2GCellByPageAjaxAction',
		type:'post',
		dataType:'text',
		success:function(raw){
			var data={};
 ...
 * @version 1.2
 */
function queryHw2GNcellData(){
	$("#progressDesc").html("");
	$(".loading_fb").text("华为2G小区邻区数据");
	$(".loading_cover").css("display", "block");
	...
	$("#searchNcellForm").ajaxSubmit({
		url:'queryHw2GNcellByPageAjaxAction',
		type:'post',
		dataType:'text',
		success:function(raw){
			var data={};
...
function displayCellData(data){
	...
	//MEA_DATE,BSC,CELL_NAME,LAC,CI
	var oriParamArr=["MEA_DATE","BSC","CELL_NAME","LAC","CI"];
	...
	for ( var i = 0; i < data.length; i++) {
		...
		for(var j = 0; j < newParamArr.length; j++){
			html+="<td>"+getValidValue(one[newParamArr[j]],'')+"</td>";
		}
		html+="</tr>";
	}
...
function displayNcellData(data){
	...
	//MEA_DATE,t.ENGNAME BSC,CELL
	var oriParamArr=["MEA_DATE","BSC","CELL","NCELL"];
...
/**
 * 
 * @title 获取2G华为小区查询所有列表信息填充
 * @author chao.xj
 * @date 2014-10-23下午4:57:58
 * @company 怡创科技
 * @version 1.2
 */
function get2GHwCellQueryAllListInfo(){
	var cityId=$("#citymenu").find("option:selected").val();
	$.ajax({
		url : 'get2GHwCellQueryPageAllListInfoAction',
		type : 'post',
		data:{'cityId':cityId},
		dataType : 'text',
...
//							console.log(listInfo[key]['CELL'][cellPara]['tabName']);
							if("MEA_DATE"==cellPara || "CELL_NAME"==cellPara || "LAC"==cellPara || "CI"==cellPara || "BSC_ID"==cellPara || "CITY_ID"==cellPara){
								continue;
							}
								
						$("#cell_targetParam").append("<option>"+cellPara+"</option>");
						$("#cell_waitforselParam").append("<option>"+cellPara+"</option>");
						...
						for (var ncellPara in listInfo[key]['NCELL']) {
							if("MEA_DATE"==ncellPara || "BSC"==ncellPara || "CELL"==ncellPara || "BSC_ID"==ncellPara || "CITY_ID"==ncellPara|| "NCELL"==ncellPara){
								continue;
							}
 ...
 * @version 1.2
 */
function get2GHwCellQueryDateAndBscListInfo(cityId,forWhat){
	$.ajax({
		url : 'get2GHwCellQueryPageDateAndBscListInfoAction',
		type : 'post',
		data:{'cityId':cityId},
		dataType : 'text',
		success : function(data) {
...
function exportHw2GCellData(){
	...
	$("#searchCellForm").ajaxSubmit({
		url:'exportHw2GCellDataAjaxForAction',
		type:'post',
		dataType:'text',
		success:function(raw){
			var data={};
...
function exportHw2GNcellData(){
	...
	$("#searchNcellForm").ajaxSubmit({
		url:'exportHw2GCellNcellDataAjaxForAction',
		type:'post',
		dataType:'text',
		success:function(raw){
			var data={};
...
function downloadDataFile(token) {
	var form = $("#downloadHwCellDataFileForm");
...
//	var cityId=$("#citymenu").find("option:selected").val();
	$.ajax({
		url:'get2GHwCellQueryPageMoreDateInfoAction',
		type:'post',
