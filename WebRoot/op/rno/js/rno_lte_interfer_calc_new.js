					...
					$("#addTask")
							.click(
									function() {
										...
										location.href = 'stepByStepOperateNewLteInterCalcTaskInfoPageForAjaxAction?cityId='
												+ cityId;
									});
					/**
					 * 任务消息页面的下一步点击事件
							...
							.click(
									function() {
										if (pciCellValCheck()) {
											...
											var calcType = $(
													"input:radio[name='calcType']:checked")
													.val();
...
function fromParamInfoToTaskInfoPage() {
	...
	$
			.ajax({
				url : 'storageNewLteInterferCalcTaskObjInfoForAjaxAction',
				data : {
				...
				complete : function() {
					...
					// 跳转新的页面
					location.href = "stepByStepOperateNewLteInterCalcTaskInfoPageForAjaxAction?taskInfoType=paramInfoBack";
				}
			});
...
// 参数配置页面的下一步点击事件
function fromParamInfoToImportFlowFilePage() {
	...
	$
			.ajax({
				...
				complete : function() {
					// hideOperTips("loadingDataDiv");
					// 跳转新的页面
					location.href = "stepByStepOperateNewLteInterCalcTaskInfoPageForAjaxAction?taskInfoType=paramInfoForward";
				}
			});
}
...
function fromImportFlowFileToParamInfoPage() {
	...
	location.href = "stepByStepOperateNewLteInterCalcTaskInfoPageForAjaxAction?taskInfoType=importFlowFileBack";
}
			...
			.ajax({
				...
				complete : function() {
					...
					if (caltype != "" && caltype != null) {
						location.href = "stepByStepOperateNewLteInterCalcTaskInfoPageForAjaxAction?taskInfoType=importFlowFileForward&isWithFlow=y&ks="
								+ ks;
					} else {
						location.href = "stepByStepOperateNewLteInterCalcTaskInfoPageForAjaxAction?taskInfoType=importFlowFileForward&isWithFlow=n&ks="
								+ ks;
					}
				}
			});
...
function fromSweepToImportFlowFilePage() {
	showOperTips("loadingDataDiv", "loadContentId", "返回上一步");
	location.href = "stepByStepOperateNewLteInterCalcTaskInfoPageForAjaxAction?taskInfoType=sweepBack";
}
// importflowfile消息页面下一步点击事件
function fromSweepToOverViewInfoPage() {
	...
	if (isUseSf
			&& isFreqAdj
			&& (!isNumeric(d1Freq, /^[1-9]\d{4}$/)
					|| !isNumeric(d2Freq, /^[1-9]\d{4}$/)
					|| d1Freq.trim().length == 0 || d2Freq.trim().length == 0)) {
		alert("请输入有效的D1D2频");
		return;
	}
	...
	$
			.ajax({
				url : 'storageNewLteInterferCalcTaskObjInfoForAjaxAction',
				data : {
					'taskInfoType' : 'sweepForward',
					'cond.filenames' : filenames,
					'cond.d1Freq' : d1Freq,
					'cond.d2Freq' : d2Freq,
					'cond.freqAdjType' : freqAdjType
				},
				dataType : 'text',
				type : 'post',
				success : function(raw) {
				...
				complete : function() {
					location.href = "stepByStepOperateNewLteInterCalcTaskInfoPageForAjaxAction?taskInfoType=sweepForward"
				}
			});
}
// overview消息页面上一步点击事件
function fromOverviewInfoToSweepPage() {
	...
	$
			.ajax({
				...
				complete : function() {
					// 跳转新的页面
					location.href = "stepByStepOperateNewLteInterCalcTaskInfoPageForAjaxAction?taskInfoType=overviewInfoBack";
				}
			});
}
...
function pciPlanAnalysis() {
	...
	$.ajax({
		url : 'submitNewPciPlanAnalysisTaskAction',
		data : {
			'lteCells' : lteCells,
			'cond.calType' : calType
		},
		type : 'post',
		dataType : 'text',
		success : function(raw) {
...
/*	if (calType == "y") {
	 ...
	 */
	fileCode = "RNO_PCI_PLAN_IMPORT_NEW";
	$("#fileCode").attr("value", "RNO_PCI_PLAN_IMPORT_NEW");
	var attachParams = {};
			...
			.ajax({
				...
				success : function(raw2) {
					...
					if (accp['flag'] == true) {
						...
						$('#submitTask').attr("disabled", "true");
						showOperTips("loadingDataDiv", "loadContentId",
								"正在提交结构分析计算任务");
			...
			.ajax({
				...
				success : function(raw2) {
					...
					if (accp['flag'] == true) {
						...
						// showOperTips("loadingDataDiv2", "loadContentId2",
						// "正在提交结构分析计算任务");
...
function cancleLteInterferCalcTask() {
	location.href = "initNewLteInterferCalcPageAction";
}
	...
	$
			.ajax({
				...
				complete : function() {
					// hideOperTips("loadingDataDiv");
					// 跳转新的页面
					location.href = "stepByStepOperateNewLteInterCalcTaskInfoPageForAjaxAction?taskInfoType=taskInfoForward";
				}
			});
}
...
function sumbitStructureForm() {
	...
	$("#structureTaskForm").ajaxSubmit({
		url : 'queryNewPciPlanAnalysisTaskByPageForAjaxAction',
		dataType : 'text',
		success : function(raw) {
 ...
 */
function getEndDate() {
	var begDateStr = $("#beginTime").val().replace(/-/g, "\/");
	var endDateStr = $("#latestAllowedTime").val().replace(/-/g, "\/");
	var begDate = new Date(begDateStr);
	var endDate = new Date(endDateStr);
	var begTimeStr = extendInt(begDate.getHours()) + ":"
	...
	if (exDateRange(endDateStr, begDateStr) > maxDateInterval) {
		...
		var sss = endDate.getFullYear() + "-" + (endDate.getMonth() + 1) + "-"
				+ endDate.getDate() + " " + begTimeStr;
		if (exDateRange(endDateStr, sss.replace(/-/g, "\/")) > maxDateInterval) {
			endDate.setDate(endDate.getDate() + 1);
			sss = endDate.getFullYear() + "-" + (endDate.getMonth() + 1) + "-"
					+ endDate.getDate() + " " + begTimeStr;
		}
		$("#beginTime").val(sss);
		beginTime.focus();
