...
function showNcsTaskQueryResult(rawdata) {
	...
	for ( var i = 0; i < list.length; i++) {
		...
		html += "<tr>";
		html +=  "<td><input type='radio' class='forcheck' name='cellStructureItem' id='structure_"
			+ one['TASK_ID'] + "' onclick='refreshCellStrutureAnalysisList(this,\""+"cellStructAnalysisListTable"+"\")'/></td>";
 ...
 */
function refreshCellStrutureAnalysisList(obj,tabName){
	var analysisList = $("#"+tabName);
	var item = $(obj);
	var id = item.attr("id");
	if(analysisList.find("tr").length > 0){
		analysisList.find("tr").eq(0).remove();
	}
	var levelName = item.parent().next("td").text();
	var cnt = item.parent().next("td").next("td").text();
	var startTime = item.parent().next("td").next("td").next("td").text();
	var html = "<tr><td><input type='hidden' value='"+id+"'/></td>";
	html += "<td>"+levelName+"</td><td>"+cnt+"</td><td>"+startTime+"</td>";
