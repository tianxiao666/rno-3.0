...
function bindEvent(){
	$("#importBtn").click(function() {
		...
		$("#err").remove();
		if ($("#fileid").val() == "") {
			$("#fileid").parent().append("<font id='err' color='red'>请选择中兴切换文件</font>");
			return;
		}
		if ($.trim($("#meatime").val()) == "") {
 ...
 * 查询中兴切换描述数据
 */
function queryZteHoDescData(){
	$("#searchNcsForm").ajaxSubmit({
		url:'queryZte4GHoDescAjaxAction',
		type:'post',
		dataType:'text',
		success:function(raw){
			var data={};
			...
			displayZteHoDescData(data['data']);
 ...
 * @param data
 */
function displayZteHoDescData(data){
	if(data==null||data==undefined){
		return;
	}
	//
