...
function bindEvent(){
	...
	//查询MR描述数据
	$("#searchNiBtn").click(function(){
		initFormPage("searchNiForm");
		queryNiDescData();
	});
 ...
 * 查询MR描述数据
 */
function queryNiDescData(){
	$(".loading_fb").text("4g-NI数据");
	$(".loading_cover").css("display", "block");
	$("#searchNiForm").ajaxSubmit({
		url:'queryNiDescAjaxAction',
		type:'post',
		dataType:'text',
		success:function(raw){
			var data={};
			...
			setFormPageInfo("searchNiForm",data['page']);
...
function displayMrDescData(data){
	...
	for ( var i = 0; i < data.length; i++) {
		...
		html+="<td>"+getValidValue(one['RECORD_CNT'],'')+"</td>";
