...
<body>
	...
	<center>
		...
		<br /> <font style="color: #31FF81;font-weight: bold;">任务信息</font><font style="font-weight: bold;">>>参数配置>>流量文件>>扫频数据>>提交任务</font> <br>
	</center>

	<input type="hidden" id="provinceIdFromSession"
		value="<s:property value="#session.MRTASKINFO.taskInfo.provinceId"/>" />
	<input type="hidden" id="cityIdFromSession"
		value="<s:property value="#session.MRTASKINFO.taskInfo.cityId"/>" />
	<input type="hidden" id="startTimeFromSession"
		value="<s:property value="#session.MRTASKINFO.taskInfo.startTime"/>" />
	<input type="hidden" id="endTimeFromSession"
		value="<s:property value="#session.MRTASKINFO.taskInfo.endTime"/>" />

	<input type="hidden" id="planTypeSession"
		value="<s:property value="#session.MRTASKINFO.taskInfo.planType"/>" />
	<input type="hidden" id="converTypeSession"
		value="<s:property value="#session.MRTASKINFO.taskInfo.converType"/>" />
	<input type="hidden" id="cosiSession"
		value="<s:property value="#session.MRTASKINFO.taskInfo.relaNumerType"/>" />
	<input type="hidden" id="CALCONCLUSTERSession"
		value="<s:property value="#session.MRTASKINFO.taskInfo.calProcedure.CALCONCLUSTER"/>" />
	<input type="hidden" id="CALCLUSTERCONSTRAINSession"
		value="<s:property value="#session.MRTASKINFO.taskInfo.calProcedure.CALCLUSTERCONSTRAIN"/>" />
	<input type="hidden" id="CALCLUSTERWEIGHTSession"
		value="<s:property value="#session.MRTASKINFO.taskInfo.calProcedure.CALCLUSTERWEIGHT"/>" />
	<input type="hidden" id="CALCELLRESSession"
		value="<s:property value="#session.MRTASKINFO.taskInfo.calProcedure.CALCELLRES"/>" />
	<input type="hidden" id="CALIDEALDISSession"
		value="<s:property value="#session.MRTASKINFO.taskInfo.calProcedure.CALIDEALDIS"/>" />
		...
		<table class="taskInfoTab" rules="none">
			<tbody>
				<tr>
					...
					<td align="left">省：<select class="required" id="provinceId">
					...
					</select> <input type="hidden" id="cityIdParam"
						value="<s:property value="cityId"/>" /> 市：<select name=""
						class="required" id="cityId2">
				...
				<tr>
					...
					<td align="left"><textarea
							style="width: 330px; height: 25px; " id="taskName"><s:property
								value="#session.MRTASKINFO.taskInfo.taskName" /></textarea> <span
						style="color:red;width:100px;font-family:华文中宋;" id="nameError"></span>
					</td>
				</tr>
				<tr>
					<td align="right">任务描述：<br> <span
						style="color:red;width:100px;font-family:华文中宋;" id="descErrorText"></span>
					</td>
					<td align="left"><br> <textarea
							style="width: 330px; height: 184px; " id="taskDescription"><s:property
								value="#session.MRTASKINFO.taskInfo.taskDesc" /></textarea> <span
						style="color:red;width:100px;font-family:华文中宋;" id="descError"></span>
					</td>
				</tr>
				<tr>
					<td align="right">测量时间<font style="color: red;">*</font>：<br>
						<span style="color:red;width:100px;font-family:华文中宋;"
						id="dateErrorText"></span>
					</td>
					<td align="left"><s:set name="begtime"
							value="new java.util.Date()" /> <input id="beginTime"
						name="cond['begTime']"
						...
						readonly class="Wdate input-text" style="width: 132px;" /> 到 <s:set
							name="endtime" value="new java.util.Date()" /> <input
						id="latestAllowedTime" name="cond['endTime']"
						...
						readonly class="Wdate input-text" style="width: 132px;" /> <span
						style="color:red;width:100px;font-family:华文中宋;" id="dateError"></span>
					</td>
				</tr>
				<tr>
					<td align="right">评估方案：</td>
					<td>方案评估1(三步法) <input type="radio" id="planType1"
						name="planType" value="ONE" 　checked="checked" />&nbsp;&nbsp;&nbsp;&nbsp;方案评估2(两步法)
				...
				<tr>
					...
					<td>方案一(根据Top差值比例) <input type="radio" id="useEriData"
						name="schemeType" value="ONE" 　checked="checked" />&nbsp;&nbsp;&nbsp;&nbsp;方案二(根据求方差)
						<input type="radio" id="useHwData" name="schemeType" value="TWO" /></td>
				</tr>
<%-- 				<tr>
					<td align="right">关联度分子：</td>
