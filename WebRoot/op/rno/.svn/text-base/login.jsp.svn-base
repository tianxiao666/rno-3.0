...
<link type="text/css" rel="stylesheet" href="login/css/login.css?version=<%=(String)session.getAttribute("version")%>"/>
<script src="jslib/jquery-1.8.2.js"></script>
<script src="login/jslib/loginSlide.js"></script>
<script>
// 设置HTML5本地存储中SelectedMenuId的值为空
				...
				<div class="login_form">
					<form:form method="post"  id="loginForm" cssClass="fm-v clearfix"  action="rnoAuthenticate.action"  htmlEscape="true">
						...
						<div class="login_formIpt" style="z-index: 1000;">
							<input type="text" id="inputName" title="请输入帐号" class="loginFormTdIpt userNameIpt" value="<%=oriUserName%>" />
						...
						<div class="login_formIpt">
							...
							<input type="text" id="passWordText" title="请输入密码" class="loginFormTdIpt" value="密码">
						</div>
						<% if(failTime>0){ %>  
						...
						<input type="hidden" id="jump" value="<%=jumpParams%>"/>
	...
	<div class="login_footer">
		<div class="footer_inner">
			...
			<div class="footer_nav">
				...
				<a href="#">客户服务</a>
				<a href="fileDownloadAction?fileName=rno_user_manual.zip" target="_blank">使用说明书下载</a>
				|<span class="copyright"><%=(String)session.getAttribute("projectName")+" "+(String)session.getAttribute("version")%></span>
				<span class="copyright"><%=(String)session.getAttribute("corporationName")%>版权所有 &copy; <%=(String)session.getAttribute("yearInterval")%></span>
			</div>
		</div>
	</div>
	<%
