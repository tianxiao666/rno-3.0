...
<script type="text/javascript" src="js/rno_channel_switch_statistics_import.js"></script>
<%DateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");%>
<script type="text/javascript">
	var $ = jQuery.noConflict();
	$(function() {
		//tab选项卡
		tab("handover_div_tab", "li", "onclick");//项目服务范围类别切换
...
function showSts(data,tabFlag,formId){
	...
	table.append(getTableHead());
		...
		//alert(new Date(value.STS_DATE));
			...
			tr += "<td><input type='checkbox' id='handover_"+value.RNO_HANDOVER_DESC_ID+"' name='loaditem' onclick='refreshHandoverAnalysisList(this,\""+"handoverAnalysisListTable"+"\")'/></td>";
			tr += "<td><span>" + value.AREANAME + "</span></td>";
...
//跳转
function showHandoverListByPage(dir,formId,tabFlag){
...
function getTableHead() {
	//console.log("来了");
	var str = '<th style="width: 10%">加载项</th>';
	str += '<th style="width: 8%">区域</th>';
	str += '<th style="width: 10%">指标日期</th>';
...
<body>
	...
	<div class="div_left_main" style="width: 100%;margin:0px;">
		<div class="div_left_content" style="min-height:300px;max-height:600px">
			<%-- <div class="div_left_top">加载小区切换统计</div> --%>
			<div style="padding: 10px">
				<div id="frame" style="border: 1px solid #ddd;overflow: hidden;float: left;width: 100%">
					<div id="handover_div_tab" class="div_tab divtab_menu150px">
						<ul>
					...
					<div class="divtab_content">
						<div id="handover_div_tab_0">
										...
										<tbody>
											<tr>
												...
												<td style="width: 50%; font-weight: normal;" colspan="0">
													...
													id="handover_provinceId" style="width:80px">
												...
												</select> 市：<select name="cityId" class="required" id="handover_cityId" style="width:80px">
												...
												</select> 区：<select name="areaId" class="required" id="handover_areaId" style="width:80px">
								...
								<div class="container-bottom" style="padding-top: 10px">
									<table style="width: 60%; margin: auto" align="center">
										<tr>
											...
											<%--<input type="button" value="导 入，并加载到分析列表" id="importAndLoadBtn" name="import" />--%>
											<input type="button" value="导    入" style="width: 90px;" id="importBtn" name="import" /> <br /></td>
							...
							<div id="handover_div_tab_1" style="display:none;">
                                    ...
                                    <tr>
                                        ...
                                        <td style="width:50%; text-align:left">
                                              省：<select name="provinceId" class="required"
												...
												</select> 市：<select name="cityId" class="required" id="handover_cityId2">
														<s:iterator value="cityAreas" id="onearea">
															<option value="<s:property value='#onearea.area_id' />">
																<s:property value="#onearea.name" />
															</option>
														</s:iterator>
												</select> 区：<select name="areaId" class="required" id="handover_areaId2" style="width:80px">
														<s:iterator value="countryAreas" id="onearea">
															<option value="<s:property value='#onearea.area_id' />">
																<s:property value="#onearea.name" />
															</option>
                                        ...
                                        <td style="width:15%; text-align:left">
                                            <%-- input name="queryCondition.staticTime" value="" type="text" style="width:90%" onFocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" readonly class="Wdate required input-text"/ --%>
                                             始:<input type="text" id="beginTime" name="date" onFocus="var dd=$dp.$('latestAllowedTime');WdatePicker({dateFmt:'yyyy-MM-dd',onpicked:function(){latestAllowedTime.focus();},maxDate:'#F{$dp.$D(\'latestAllowedTime\')}'})" readonly class="Wdate required input-text"  style="width: 70px"/>
                                                 
                                        </td>
                                        <td>
                                        止:<input type="text" id="latestAllowedTime" name="date" onFocus="WdatePicker({dateFmt:'yyyy-MM-dd',minDate:'#F{$dp.$D(\'beginTime\')}'})" value="<%=dateFormat.format(new Date())%>" readonly class="Wdate required input-text"  style="width: 70px"/>
                                        
                                        </td>
                                    </tr>                           
                                    <tr>
                                        <td style="width:10%; text-align:right" colspan="4"><%-- 重要级别： --%>
                                        </td>
                        	...
                        	<div style="padding-top:0px;width:100%;max-height:300px;overflow-y:auto">
                            <form id="form1" name="form1" method="post">
                            	<table id="tab_2_queryResultTab" class="greystyle-standard" style="width:100%">
									<thead >
							...
							<div class="paging_div" style="border: 1px solid #ddd">
								...
								<a class="paging_link page-first" title="首页"
									onclick="showHandoverListByPage('first','form_tab_2','tab_2_')"></a>
								<a class="paging_link page-prev" title="上一页"
									onclick="showHandoverListByPage('back','form_tab_2','tab_2_')"></a> 第
								<input type="text" id="tab_2_showCurrentPage" class="paging_input_text" value="0" />
								页/<em id="tab_2_emTotalPageCnt">0</em>页
								<a class="paging_link page-go" title="GO" onclick="showHandoverListByPage('num','form_tab_2','tab_2_')">GO</a>
								<a class="paging_link page-next" title="下一页"
									onclick="showHandoverListByPage('next','form_tab_2','tab_2_')"></a>
								<a class="paging_link page-last" title="末页"
									onclick="showHandoverListByPage('last','form_tab_2','tab_2_')"></a>
							</div>   
                        	</div>
