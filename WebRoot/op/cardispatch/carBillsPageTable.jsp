...
<table id="resultListTable" class="search_result_table">  
	<tbody>
		...
		<s:iterator id="map" value="pageList" status="st">
				...
				<td><a target="_blank" href="getReadingAndBillsAction?id=<s:property value="#map.fuel_bills_id"/>&distinction=Bills"><s:property value="#map.fuel_bills"/></a></td>
				...
				<s:if test="#map.status == 1">
					<td>正常</td>
					<td><a class="delete_people" href="javascript:deleteReadingOrBills(<s:property value="#map.fuel_bills_id"/>);" ></a></td>
				</s:if>
				<s:else>
