 ...
 * @param {Object} rolekind
 */
function modifyRole(type){
	if($("#_hidden_"+type+"_orgrole_role_id").val()=="" ){
		alert("请先选择要修改的角色。");
		return;
	}
	$("#"+type+"_orgrole_modify_role_form").validate({submitHandler: function(form) { 
		$("#"+type+"_orgrole_modify_role_form").ajaxSubmit( {
	...
	//			alert(obj['flag'],"  "+decodeURIComponent(obj['msg']));
				if(obj['flag']==true){
					...
					//更新几个地方的信息 
					//更新本tab的角色li
					$("#"+$("#_hidden_"+type+"_orgrole_li_id").val()).replaceWith($(obj['msg']));
				}else{
					alert("修改失败。原因："+obj['msg']);
					//重新触发该li的点击事件，
					$("#"+$("#_hidden_"+type+"+orgrole_li_id").val()).trigger("click");
 ...
 * @param {Object} rolekind
 */
function deleteRole(type){
	if(confirm("你确定要删除该角色？该操作不可复原！")){
		$.ajax({
			type:'POST',
			url:'ajaxDeleteRoleAction',
			data:{'roleId':$("#_hidden_"+type+"_orgrole_role_id").val()},
			success:function(d){
				var data=eval("("+d+")");
				if(data['flag']==true){
					//清除树控件列表
					$("#"+type+"_orgrole_role_ul").children("li[id="+$("#_hidden_"+type+"_orgrole_li_id").val()+"]").remove();
					
					//清除其他信息
					if($("#"+type+"_orgrole_role_ul").children("li").length>0){
					  $("#"+type+"_orgrole_role_ul").children("li:first").trigger('click');
					}else{
...
function addRole(type){
	$("#"+type+"_orgrole_add_role_form").validate({submitHandler: function(form) {
		$("#"+type+"_orgrole_add_role_form").ajaxSubmit({
			dataType:'text',
			success:function(data){
			  var obj=eval("("+data+")");
			  if(obj['flag']==true){
				  ...
				  clear(type);
				  var newLi=$(obj['msg']);
				  $("#"+type+"_orgrole_role_ul").append(newLi);
				  
				  $("#"+type+"_orgrole_role_"+obj['roleId']).click();
