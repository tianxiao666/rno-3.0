...
.lititle {
	...
	width: 100px;
			...
			<script type="text/javascript" src="js/statements.js">
</script>
			...
			<script type="text/javascript"
				src="../cardispatch/js/util/dateutil.js">
</script>
			<script type="text/javascript"
				src="../cardispatch/js/util/objutil.js">
</script>
	...
	//报表放大按钮
		...
		$(".time_tool_left").click(
				function() {
					...
					$("#year_span").text(year);
					$("#month_span").text(month);
					orgTitle = "车辆里程统计(" + thisOrgName + ","
							+ $(".time_tool_main").text() + ")";
					createReportByBiz(thisOrgId);
				});
		$(".time_tool_right").click(
				function() {
					now.addMonths(1);
					year = now.getFullYear();
					month = now.getMonth() + 1;
...
function createReportByBiz(orgId) {
	...
	$.ajax( {
		...
		"success" : function(result) {
			...
			var carGpsMileageArr = new Array();
			var carCountArr = new Array();
			for ( var key in data) {
				columnName.push(key);
				var info = data[key];
				for ( var key_info in info) {
					var num = 0;
					if (parseFloat(info[key_info]) != undefined) {
						num = parseInt(info[key_info]);
					}
					if (key_info == "info") {
						orgId_info[key] = info[key_info];
					} else if (key_info == "totalMileage") {
						carMileageArr.push(num);
					} else if (key_info == "totalGpsMileage") {
						carGpsMileageArr.push(num);
					} else if (key_info == "carCount") {
						carCountArr.push(num);
			...
			//X标题数组
			//数据
			columnAndSplineBasic1(orgTitle, "(点击图形区域,可向下钻取)", "里程（公里）", "车辆数量",
					"container", columnName, arr);
			//columnAndSplineBasic1(orgTitle, "(点击图形区域,可向下钻取)", "里程（公里）", "车辆数量", "container_copy_big", columnName, arr);
...
function createRoundReportByBiz() {
	...
	$
			.ajax( {
				"url" : "carCensusReport_ajax!censusCarCountMileageForRoundInTimesAction.action",
				"type" : "post",
				"data" : {
					"months" : obj2String(yearMonths)
				},
				"success" : function(result) {
					orgId_info = {};
					var data = eval("(" + result + ")");
				...
				for ( var key in data) {
					...
					for ( var key_info in info) {
						if (key_info == "info") {
							...
							carMileageArr.push(parseFloat(info[key_info]));
						} else if (key_info == "totalGpsMileage") {
							carGpsMileageArr.push(parseFloat(info[key_info]));
						} else if (key_info == "carCount") {
				...
				carGpsMileage["data"] = carGpsMileageArr;
				carCount["data"] = carCountArr;
				var arr = [ carMileage, carGpsMileage, carCount ];
				//yAxisTitle - y提示
				//容器id
				//X标题数组
				//数据

				columnAndSplineBasic1(orgTitle, "(点击图形区域,可向下钻取)", "车辆数量",
						"里程（公里）", "container2", columnName, arr)
				columnAndSplineBasic1(orgTitle, "(点击图形区域,可向下钻取)", "车辆数量",
						"里程（公里）", "container2_copy_big", columnName, arr)
				//报表评论
				getReportComment("carOilRoundReportComment", "org", thisOrgId,
						null, "reportComment2");
				hideCover();
			}
			});
...
function createReportDataTable() {
	...
	for ( var key in mileageData) {
		var info = mileageData[key];
		var tr = $("<tr/>").appendTo($(oil_stationTable));
		var bizNameTd = $("<td/>").appendTo($(tr)).text(key);
		var oilMoneyTd = $("<td/>").appendTo($(tr)).text(
				getObjectData(info, "totalMileage", "0"));
		var oilNumTd = $("<td/>").appendTo($(tr)).text(
				getObjectData(info, "totalGpsMileage", "0"));
		var oilUseNumTd = $("<td/>").appendTo($(tr)).text(
				getObjectData(info, "carCount", "0"));
	}
}
...
function getObjectData(obj, key, def) {
	...
	if (key instanceof Array) {
		...
		result = obj[key] == undefined || obj[key] == null
				|| obj[key] == "null" ? def : obj[key];
	}
	return result;
}
...
function addDivChar(div) {
	var report_statements_full = document
			.getElementById("report_statements_full_newNode");
	...
	<body>
		...
		<div id="statements_content">
			...
			<div class="statements_right" id="statements_right_div">
				...
				<div class="statements_search_date">
					...
					<span class="time_tool_main"> <span id="year_span"></span>年
						<span id="month_span"></span>月 </span>
				...
				<div id="compareTab" class="compareTab_menu">
					<ul class="clearfix">
						...
						<li title="不同统计周期的趋势分析(通常是按月)">
							环比
						</li>
						<li class="lititle" title="不同年度的同期趋势分析">
							同比
						</li>
					</ul>
				</div>
				<div class="compare_info">
					<div id="compareTab_0">
						<div class="statements_tab">
							<div class="tab_menu">
								<ul>
									<li class="ontab first_tab" id="reload_contrast_biz">
										按组织
									</li>
								</ul>
							...
							<div class="tab_container">
								<div class="tab_content">
									...
									<div class="statements_select1">
										<span id="statements_back" class="statements_back" style=""
											title="返回上级"></span>
										<select>
								...
								<div class="statements_table_top" style="height: 26px;"
									id="statements_table_top">
									<span style="float: left;"><a
										onclick="showOrHideStationTableDiv(this);"><em>[+]</em>展开表格</a>
									</span>
								</div>
								<%--表单位置--%>
								<div class="statements_main_table" id="stationTableDiv"
									style="display: none;">
		...
		<%-- 报表放大 --%>
		<div id="container_copy_big"
			style="height: 500px; width: 1000px; display: none;"></div>
		<div id="container2_copy_big"
			style="height: 500px; width: 1000px; display: none;"></div>
	</body>
</html>
