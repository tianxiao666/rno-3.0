...
var isO = false;
	$(function(){
				...
				var orgId = $("#orgId").val();
				getWANByOrg(orgId,dates[0],dates[1]);
	});
	
	
function getWANByArea(orgId,beginTime,endTime){
		...
		var url = "getWANByAreaAction";
		var countext = "";
		var categories = new Array();
		var series = "";
		var pName = "";
		var cell = "{ name: '小区', data: [ ";
		...
		var tdB = "{ name: 'TD基站', data: [ ";
			var params = {orgId:orgId,beginTime:beginTime,endTime:endTime};
				...
				gsmB = gsmB + "]},";
				tdB = tdB + "]}";
				series = eval("[" +cell + gsmB + tdB + "]");
				$("#di").html(countext);
				//var title = "无线分类统计("+pName + ","+$("#DateTimeSpan").text()+")";
				var title = "无线分类统计("+pName + ")";
			...
			getReportComment("WAN", "Area", orgId, beginTime);
}


function getWANByAreaId(areaId,beginTime,endTime){
		$("#columnBasicDiv").html("<div  style=\"text-align: center;   padding-top: 20px;\" valt=\"Char\"><img width=\"20\" height=\"20\" style=\"vertical-align:middle\" src=\"image/loading.gif\"><em>数据加载中，请稍侯...</em></div>");
		var url = "getWANByAreaIdAction";
		var countext = "";
		var categories = new Array();
		var series = "";
		var pName = "";
		...
		var tdB = "{ name: 'TD基站', data: [ ";
			...
			getReportComment("WAN", "Area", areaId, beginTime);
}


function getWANByOrg(orgId,beginTime,endTime){
		$("#columnBasicDiv").html("<div  style=\"text-align: center;  padding-top: 20px;\" valt=\"Char\"><img width=\"20\" height=\"20\" style=\"vertical-align:middle\" src=\"image/loading.gif\"><em>数据加载中，请稍侯...</em></div>");
		var url = "getWANByOrgAction";
		var countext = "";
		var categories = new Array();
		var series = "";
		var pName = "";
		...
		var tdB = "{ name: 'TD基站', data: [ ";
			...
			getReportComment("WAN", "Org", orgId, beginTime);
}


function getWANByOrgId(orgId,beginTime,endTime){
		$("#columnBasicDiv").html("<div  style=\"text-align: center;   padding-top: 20px;\" valt=\"Char\"><img width=\"20\" height=\"20\" style=\"vertical-align:middle\" src=\"image/loading.gif\"><em>数据加载中，请稍侯...</em></div>");
		var url = "getWANByOrgIdAction";
		var countext = "";
		var categories = new Array();
		var series = "";
		var pName = "";
...
function getReportByorginst(me){
	...
	if($(me).text().replace(" ","") == "按组织"){
		...
		getWANByOrg(orgId,beginTime,endTime);
		...
		getWANByArea(orgId,beginTime,endTime);
		...
		var thisAreaId = $("#thisAreaId").val();
		$("#returnAreaId").val(thisAreaId);
		getWANByProject(orgId,beginTime,endTime)
	}
...
function clickShowMore(divId,name){
	...
	if($(".ontab").text().replace(" ","") == "按组织"){
			...
			getWANByOrgId(thisorgId,null,null);
	}if($(".ontab").text().replace(" ","") == "按地域"){
			if(thisorgId != null && thisorgId != ""){
				$("#statements_back").show();
				$("#returnAreaId").val(thisorgId);
			}
			getWANByAreaId(thisorgId,null,null);
	}if($(".ontab").text().replace(" ","") == "按项目"){
			if(thisorgId != null && thisorgId != ""){
				$("#returnAreaId").val(thisorgId);
			}
			//getWANByProject(thisorgId,null,null);
	}
...
function clickGetUrgentRepairworkorderReport(){
	...
	if(isO == true){
		if($(".ontab").text().replace(" ","") == "按组织"){
			getWANByOrgId(thisorgId,beginTime,endTime);
		}if($(".ontab").text().replace(" ","") == "按地域"){
			getWANByAreaId(thisorgId,beginTime,endTime);
		}if($(".ontab").text().replace(" ","") == "按项目"){
			getWANByProject(thisorgId,beginTime,endTime);
		}
...
function clickReturnReport(){
	...
	if($(".selected").text().replace(" ","") == "对比"){
		if($(".ontab").text().replace(" ","") == "按组织"){
			...
			getWANByOrgIdTopOrg(returnOrgId,beginTime,endTime);
		}if($(".ontab").text().replace(" ","") == "按地域"){
			...
			getWANByAreaIdTopOrg(returnAreaId,beginTime,endTime);
		}if($(".ontab").text().replace(" ","") == "按项目"){
			var returnAreaId = $("#returnAreaId").val();
			var thisAreaId = $("#thisAreaId").val();
			if(returnAreaId == thisAreaId){
			...
			getWANByProject(returnAreaId,beginTime,endTime);
		}
...
function getWANByAreaIdTopOrg(areaId,beginTime,endTime){
		$("#columnBasicDiv").html("<div  style=\"text-align: center;   padding-top: 20px;\" valt=\"Char\"><img width=\"20\" height=\"20\" style=\"vertical-align:middle\" src=\"image/loading.gif\"><em>数据加载中，请稍侯...</em></div>");
		var url = "getWANByAreaIdAndTopOrgAction";
		var countext = "";
		var categories = new Array();
		var series = "";
		var pName = "";
		...
		var tdB = "{ name: 'TD基站', data: [ ";
			var params = {areaId:areaId,beginTime:beginTime,endTime:endTime};
				...
				getReportComment("WAN", "Area", $("#returnAreaId").val(), beginTime);
			},'json');
}

function getWANByOrgIdTopOrg(orgId,beginTime,endTime){
		$("#columnBasicDiv").html("<div  style=\"text-align: center;   padding-top: 20px;\" valt=\"Char\"><img width=\"20\" height=\"20\" style=\"vertical-align:middle\" src=\"image/loading.gif\"><em>数据加载中，请稍侯...</em></div>");
		var url = "getWANByOrgIdAndTopOrgAction";
		var countext = "";
		var categories = new Array();
		var series = "";
		var pName = "";
		...
		var tdB = "{ name: 'TD基站', data: [ ";
			var params = {orgId:orgId,beginTime:beginTime,endTime:endTime};
				...
				getReportComment("WAN", "Org", $("#returnOrgId").val(), beginTime);
			},'json');
}
...
function getWANByProject(orgId,beginTime,endTime){
		$("#columnBasicDiv").html("<div  style=\"text-align: center;   padding-top: 20px;\" valt=\"Char\"><img width=\"20\" height=\"20\" style=\"vertical-align:middle\" src=\"image/loading.gif\"><em>数据加载中，请稍侯...</em></div>");
		var url = "getWANByOrgIdProjectAction";
		var countext = "";
		var categories = new Array();
		var series = "";
		var pName = "";
		...
		var tdB = "{ name: 'TD基站', data: [ ";
			...
			getReportComment("WANProject", "Org", orgId, beginTime);
}
