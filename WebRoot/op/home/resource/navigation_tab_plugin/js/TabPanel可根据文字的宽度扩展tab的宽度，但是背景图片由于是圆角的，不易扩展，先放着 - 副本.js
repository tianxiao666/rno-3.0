...
TabPanel = function(config){
  ...
  this.tabWidth = 105 + 4;//样式.tabpanel_mover li中width+4
...
TabPanel.prototype = {
  ...
  addTab : function(tabitem){
    ...
    else if(this.scrollFinish)
    {
     ...
     var myWidth= textSize("12px",tabitem.title).width;
    // console.log("my width="+myWidth);
     // console.log("1--"+title.width());
      title.appendTo(tab);
      //console.log("2--"+title.width());
	  var wFix = tabitem.closable==false ? 0 : 5;
      if(tabitem.icon) {
        title.addClass('icon_title');
        //console.log("3--"+title.width());
        ...
        if(myWidth>title.width()){
          title.width(myWidth);
          tab.width(myWidth+5+wFix);
        }
      }
      ...
      tab.bind('click', function(position){
        return function(){
          ...
          //暂时将事件写死在这此
          userWorkZoneSiteAction(tabitem.id,tabitem.parentId);
        };
      }(this.tabs.length-1));
