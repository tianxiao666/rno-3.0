...
TabPanel = function(config){
  ...
  this.tabWidth = 108;//105 + 4;//样式.tabpanel_mover li中width+4 //不够，改为110
  this.fixNum = 0;//16;//右边补丁2像素(不够，改为16）
  this.scrollFinish = true;//是否滚动完毕
  this.maxLength = config.maxLength || -1;//最大选项卡数量
  this.maxzindex = 0;//层高
  
  this.maxTabWidth=0;//最大的tab宽度
  this.init();//初始化
  this.moveToSee(this.active);//将激活的tab移动至可见
};
...
TabPanel.prototype = {
  ...
  //向左移动
  moveLeft : function(){
  //alert("this.maxMove=="+this.maxMove);
    if(this.scrollFinish) //由于调用的时候，用的tabEntity.moveLeft()，所以， 这个this指的是最大的那个tabpanel
    {
      this.disableScroll();
  ...
  update : function(){
    ...
    //计算最大移动量
    //alert("this.tabpanel_mover.children().length="+this.tabpanel_mover.children().length+",this.tabWidth="+this.tabWidth+",cWidth="+cWidth+",this.fixNum="+this.fixNum);
    //alert("(this.tabpanel_mover.children().length*this.tabWidth) - cWidth + this.fixNum="+(this.tabpanel_mover.children().length*this.tabWidth) - cWidth + this.fixNum);
    this.maxMove = (this.tabpanel_mover.children().length*this.tabWidth) - cWidth + this.fixNum;
    //if(this.tabpanel_mover.children().length==8)
    //alert(this.maxMove);
  },
  ...
  addTab : function(tabitem){
    ...
    else if(this.scrollFinish)
    {
      ...
      //gmh 2012-4-23 add begin
      //如果title文本里带有 class=xxx，则认为需要增加一个<span class=xxx/>的元素到tab的title div里
      var temptitle=tabitem.title;
      var inddd=temptitle.indexOf('class');
      if(inddd>=0){
        temptitle=temptitle.slice(0,inddd);
        var whatclass=tabitem.title.slice(inddd);
        whatclass=whatclass.slice(whatclass.indexOf("=")+1);
        var sp=$("<span class='"+whatclass+"'></span>");
        var newtext = document.createTextNode(temptitle);
        //title.text(temptitle);
        title.append(newtext);
        sp.appendTo(title);
        title.appendTo(tab);
      }
      //gmh 2012-4-23 add end
      else{
        title.text(temptitle);
        title.appendTo(tab);
      }
      ...
      tab.bind('click', function(position){
        return function(){
          ...
          //alert(tabitem.parentId);       
          //暂时将事件写死在这此
          userWorkZoneSiteAction(tabitem.id,temptitle,tabitem.parentId);
        };
      }(this.tabs.length-1));
