...
function getStaffsByOrgId(orgSubstanceId, page) {
	...
	$.ajax( {
		...
		var tr = "";
		$.each(staffList, function(key, object) {
			var staffEnrollDay = object.staff.staffEnrollDay.toString().split(
					" ", 1);
			tr = tr + "<tr>";
			tr = tr + "<td style='text-align: left'>" + object.organizationName
					+ "</td>";
			tr = tr + "<td style='text-align: left'>" +object.staff.name+ "</td>";
			tr = tr + "<td style='text-align: left'>" + object.staff.staffCode
					+ "</td>";
			...
			tr = tr + "<td style='text-align: left' class='tdr'>"
					+ "<a href='staffDetail.html?id="
					+ object.staff.substanceId + "'>详细</a>" + "</td>";
			tr = tr + "<tr>";
...
function showStaff(currentPage) {
	...
	$.ajax( {
		...
		tr = tr
				+ foots("queryStaffAction", page.currentPage, page.totalPage,
						page.prePage, page.nextPage, page.pageSize);
		$(tr).appendTo("#tableQuery");
...
function foots(actionName, currentPage, totalPage, prePage, nextPage, pageSize) {
	if (totalPage > 0) {
		...
		foot = foot + "<a id='prePage' href=JavaScript:prePage('" + actionName
				+ "'," + prePage + ")>上一页 </a>&nbsp;&nbsp;";
		foot = foot
				+ "<select id='selectPage'   onchange=JavaScript:selectPage('"
				+ actionName
		...
		foot = foot + "</select>";
		foot = foot + "&nbsp;&nbsp;";
		foot = foot + "<a id='nextPage' href=JavaScript:nextPage('"
				+ actionName + "'," + nextPage + ")>下一页 </a>&nbsp;&nbsp;";
