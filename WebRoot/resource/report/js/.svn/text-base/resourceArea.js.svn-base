	...
	$.ajax({
		...
		"success" : function(result){
			...
			for ( var i = 0 ; i < user_area_list.length ; i++ ) {
				...
				user_area_id_list.push( parseInt(info["area_id"]) );
			}
			loadUserArea2();
		}
	});
...
function cityClick (widget) {
	...
	if(isInArea(c.id+"")){
		$("#tree-ul").show();
		$("#tree-ul").siblings().remove();
		$("#areaId").val(c.id);
	 	$("#areaType").val(c._entityType);
	 	$("#repertTypeareaName").val(c.name);
	 	$("input[name='rdoChooseResChild']").val(c.id);
		$("#logicalresTreeRootName").text(c.name);
		$("#logicalresTreeRootName").next().val(c.id);
		$("#logicalresTreeRootName").next().next().val(c._entityType);
		$("#treeImg").attr("src","image/plus.gif");
		$("#treediv").html("");
	}else{
		$("#tree-ul").hide();
		$("#tree-ul").siblings().remove();
		$("#tree-ul").after("<div style='width: 250px; height: 300px;'>当前选择区域不在统计权限范围!</div>")
		$("#areaId").val("");
	 	$("#areaType").val("");
	 	$("#repertTypeareaName").val("");
	 	$("input[name='rdoChooseResChild']").val("");
		$("#logicalresTreeRootName").text("");
		$("#logicalresTreeRootName").next().val("");
		$("#logicalresTreeRootName").next().next().val("");
		$("#treeImg").attr("src","image/plus.gif");
		$("#treediv").html("");
	}
