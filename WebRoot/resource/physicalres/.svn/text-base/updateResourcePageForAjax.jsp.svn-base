...
function submitAction(){
	$("#operaForm").attr("action","updateResourceNewAction");
	$("#operaForm").submit();
}
...
<input type="hidden" name="updatedEntityType" value="${currentEntityType}">
<input type="hidden" name="oldParentResEntityType" value="${parentEntityType }">
<input type="hidden" name="oldParentResEntityId" value="${parentEntityId }">
<input type="hidden" name="newParentResEntityId" value="">
<input type="hidden" name="newParentResEntityType" value="">
<div class="dialog" style=" margin-left:-350px; margin-top:100px;">
    <div class="dialog_header">
        <div class="dialog_title">修改：${chineseName }</div>
					                    ...
					                    <td>
					                    	...
					                    	<s:if test="attrTypeMap.get(#map.key).indexOf('Integer') > -1">
						                    	...
						                    	<s:if test="'true'.equals(nullableMap.get(#map.key))">
							                    			...
							                    			<s:iterator id="dropdownVal" value="dropdownListMap.get(#map.key)">
								                    			...
								                    			<s:elseif test='#dropdownVal=="否"'>
								                    				<s:if test='"0"==#map.value'>
								                    						<option  value="0" selected>${dropdownVal}</option>   
								                    				</s:if>   
								                    				<s:else>
								                    			...
								                    			<s:else>
								                    				<s:if test="#dropdownVal.equals(#map.value)">
								                    						<option value="${dropdownVal}" selected>${dropdownVal}</option>
								                    				</s:if>   
								                    				<s:else>
						                    		...
						                    		<s:else>
						                    			<input title='请填写：整数，如“12”' validateDigit="#${param.currentEntityType}_${key}" id="${param.currentEntityType}_${key}" name="${param.currentEntityType}#${key}" type="text" value="${value}" />
						                    		</s:else>
						                    	</s:if>
						                    	<s:else>
						                    		<s:if test="dropdownListMap.get(#map.key) != null">
						                    			<select class="input_select required" name="${param.currentEntityType}#${key}">
						                    		...
						                    		<s:else>
						                    			<input title='请填写：整数，如“12”' class="required" validateDigit="#${param.currentEntityType}_${key}" id="${param.currentEntityType}_${key}" name="${param.currentEntityType}#${key}" type="text" value="${value}"/>
						                    		</s:else>
						                    		<i style="color:red">*</i>
						                    	</s:else>
					                    	...
					                    	<s:elseif test="attrTypeMap.get(#map.key).indexOf('Double') > -1  || attrTypeMap.get(#map.key).indexOf('Float') > -1">
						                    	...
						                    	<s:if test="'true'.equals(nullableMap.get(#map.key))">
						                    		...
						                    		<s:else>
						                    			<input title='请填写：数值，如“1232.23”，“12.03”' class="number" name="${param.currentEntityType}#${key}" type="text" value="${value}"/>
						                    		</s:else>
						                    	</s:if>
						                    	<s:else>
						                    		<s:if test="dropdownListMap.get(#map.key) != null">
						                    		...
						                    		<s:else>
						                    			<input  title='请填写：数值，如“1232.23”，“12.03”' class="number required" name="${param.currentEntityType}#${key}" type="text" value="${value}"/>
						                    		</s:else>
						                    		<i style='color:red'>*</i>
						                    	</s:else>
					                    	...
					                    	<s:elseif test="attrTypeMap.get(#map.key).indexOf('Date') > -1">
					                    		<%-- 判断属性的非空情况 --%>
						                    	<s:if test="'true'.equals(nullableMap.get(#map.key))">
						                    		<input title='请填写：日期时间，如“2012-10-09”，“2012-10-09 11:02:30”' readonly="readonly" name="${param.currentEntityType}#${key}" type="text" 
					                    			class="input_text" id="time<s:property value='#st.index' />"  value="${value}"  onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})"/>
						                    	</s:if>
						                    	<s:else>
						                    		<input title='请填写：日期时间，如“2012-10-09”，“2012-10-09 11:02:30”' readonly="readonly" name="${param.currentEntityType}#${key}" type="text" 
					                    			class="input_text required" id="time<s:property value='#st.index' />" value="${value}"   onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})"/>
						                    		<i class="redStar">*</i>
						                    	</s:else>
						                    	...
						                    	<s:if test="'true'.equals(nullableMap.get(#map.key))">
						                    		...
						                    		<s:else>
						                    			<s:if test="attrLengthMap.get(#map.key)>=100">
							                    				<textarea rows="5" title='请填写：字符串' name="${param.currentEntityType}#${key}">${value}</textarea>
							                    			</s:if>
							                    			<s:else>
							                    				<input title='请填写：字符串' name="${param.currentEntityType}#${key}" type="text"  value="${value}"/>							                    			
							                    			</s:else>
						                    		</s:else>
						                    	</s:if>
						                    	<s:else>
						                    		...
						                    		<s:else>
						                    			<s:if test="attrLengthMap.get(#map.key)>=100">
							                    				<textarea rows="5" title='请填写：字符串' class="required" name="${param.currentEntityType}#${key}">${value}</textarea>
							                    		</s:if>
							                    		<s:else>
							                    				<input title='请填写：字符串' class="required" name="${param.currentEntityType}#${key}" type="text" value="${value}"/>							                    			
							                    		</s:else>
						                    		</s:else>
						                    		<i style="color:red">*</i>
						                    	</s:else>
