...
function cityClick (widget) {
	...
	if(isInArea(c.id+"")){
		...
		$("#areaId").val(c.id);
		$("input[name='areaId']").val(c.id);
		$("#logicalresTreeId").val(c.id);
		$("#logicalresTreeName").val(c.name);
		$("#treeRootName").text(c.name);
		$("#treeRootName").next().val(c.id);
		$("#treeRootName").next().next().val(c._entityType);
	}else{
		$("#tree").hide();
		$("#tree").siblings().remove();
		$("#tree").after("<div style='width: 250px; height: 300px;'>当前选择区域不在导入资源权限范围!</div>")
		$("#areaId").val("");
		$("input[name='areaId']").val("");
		$("#logicalresTreeId").val("");
		$("#logicalresTreeName").val("");
		$("#treeRootName").text("");
		$("#treeRootName").next().val("");
		$("#treeRootName").next().next().val("");
	}
