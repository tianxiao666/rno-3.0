...
public class CommandSaveOrUpdateAppEntity implements Command {

	private ApplicationEntity entity;

	public CommandSaveOrUpdateAppEntity(ApplicationEntity entity) {
		this.entity = entity;
	}
	/**
	...
	public Integer doExecution(ContextFactory contextFactory) {
		...
		if (beList == null) {
			...
			if(mp!=null&&!mp.isEmpty()){
				sqlString = "insert into "+entity.getValue("_entityType")+"("+mp.get("attrStr")+") values("+mp.get("valueStr")+")";
				//System.out.println("sqlString:"+sqlString);
				sc = context.createSqlContainer(sqlString);
				result = context.executeInsertSQL(sc, entity.getValue("_entityType")+"");
			}else{
				result=0;
			}
			//result = context.insert(entity);
			log.debug("库中不存在该数据对象，插入该数据对象。操作结果：" + result);
		} else {
