package com.iscreate.plat.workflow.serviceaccess.impl;
...
import com.iscreate.plat.workflow.processor.*;
...
public class ServiceBeanImpl implements ServiceBean {
	...
	public List<FlowInfo> getMyApplication(String userId) throws WFException {
		ExtDataConfigBeanImpl bean = (ExtDataConfigBeanImpl) FlowBeanFactory
				.getFlowBean(BeanType.Bean_Type_ExtDataConfig);
		List list = bean.getNodes();// 提取所有的节点
		HashMap map = new HashMap();
		Iterator iterator = bean.nodeUsers.keySet().iterator();// 用户节点
		for (int i = 0; i < list.size(); i++) {
			FlowNodeInfo nodeInfo = (FlowNodeInfo) list.get(i);
			if (nodeInfo.node_type.equals(Constant.NodeType.NODE_TYPE0)) {
				while (iterator.hasNext()) {
					FlowNodeUserRel rel = (FlowNodeUserRel) bean.nodeUsers
							.get(iterator.next());
					if (nodeInfo.node_id.equals(rel.node_id)
							&& rel.node_user_Id.equals(userId))
						map.put(nodeInfo.flow_Id, nodeInfo);
				}
			}
		}
		...
		while (iterator.hasNext()) {
			arrayList
					.add((FlowInfo) bean.getFlowInfo((String) iterator.next()));
		}
		return arrayList;
	}
	...
	public List<FlowTaskInfo> getWaitFlowTask(String userId) throws WFException {
		...
		try {
			List list = session.createQuery(
					"  from FlowTaskInfo a where a.acceptorUserId='" + userId
							+ "'  and  a.taskStatus=? ").list();
			return list;
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	public List<FlowTaskInfo> taskTracking(String instanceId)
		...
		try {
			List list = session.createQuery(
					"  from FlowTaskInfo a where a.instanceId='" + instanceId
							+ "' ").list();
			return list;
		} catch (Exception e) {
			e.printStackTrace();
	...
	public String deployWorkflow(String classPathResourceName, String userId)
			throws WFException {
		FlowProcessorBean FlowProcessorBean = (FlowProcessorBean) FlowBeanFactory
				.getFlowBean(BeanType.Bean_Type_FlowProcessor);
		FlowInfo iscreateFlowDefinition = new FlowInfo();
		return FlowProcessorBean.deployWorkflow(classPathResourceName,
				iscreateFlowDefinition, userId);
	}
	...
	public String deployWorkflow(ZipInputStream zis, String userId)
		...
		FlowProcessorBean FlowProcessorBean = (FlowProcessorBean) FlowBeanFactory
				.getFlowBean(BeanType.Bean_Type_FlowProcessor);
		FlowInfo iscreateFlowDefinition = new FlowInfo();
		return FlowProcessorBean.deployWorkflow(zis, iscreateFlowDefinition,
				userId);
	}
	...
	public boolean unDeployWorkflow(String flowId, String userId)
		...
		FlowProcessorBean FlowProcessorBean = (FlowProcessorBean) FlowBeanFactory
				.getFlowBean(BeanType.Bean_Type_FlowProcessor);
		FlowInfo iscreateFlowDefinition = new FlowInfo();
		FlowProcessorBean.unDeployWorkflow(flowId, userId);
		return false;
	}
	...
	public String startWorkflow(String flowId, String userId,
		...
		ExtDataConfigBeanImpl bean = (ExtDataConfigBeanImpl) FlowBeanFactory
		.getFlowBean(BeanType.Bean_Type_ExtDataConfig);
		FlowInfo flowInfo=(FlowInfo)bean.flows.get(flowId);
		
		FlowInstanceInfo flowInstanceInfo = new FlowInstanceInfo();
		flowInstanceInfo.setObject_id(object_id);
		flowInstanceInfo.setInstance_type(flowInfo.instance_Type);
		return FlowProcessorBean.startWorkflow(flowId, userId, starterType,
				starterId, flowInstanceInfo, variables);
	}
	...
	public String startWorkflow(String flowId, String userId,
		...
		flowInstanceInfo.setInstance_type(flowInfo.instance_Type);
		return FlowProcessorBean.startWorkflow(flowId, userId, starterType,
				starterId, flowInstanceInfo, variables,endStarter,endOutcome,endVariables);
	}
	/***************************************************************************
	 * 停止一条运行流程
	 * 
	...
	public boolean endWorkflowInstance(String instanceId, String userId,
		...
		FlowProcessorBean FlowProcessorBean = (FlowProcessorBean) FlowBeanFactory
		.getFlowBean(BeanType.Bean_Type_FlowProcessor);
		return FlowProcessorBean.endWorkflowInstance(instanceId, userId, endState);
	}
	...
	public boolean revokeWorkflowInstance(String flowInstanceId, String userId,
		...
		FlowProcessorBean FlowProcessorBean = (FlowProcessorBean) FlowBeanFactory
		.getFlowBean(BeanType.Bean_Type_FlowProcessor);
		return FlowProcessorBean.revokeWorkflowInstance(flowInstanceId, userId, description);
	}
	...
	public boolean completeTask(String taskId, String outcome, String userId,
		...
		FlowProcessorBean FlowProcessorBean = (FlowProcessorBean) FlowBeanFactory
		.getFlowBean(BeanType.Bean_Type_FlowProcessor);
		return FlowProcessorBean.completeTask(taskId, outcome, userId, variables);
	}
	...
	public boolean rejectTask(String taskId, String userId) throws WFException {
		FlowProcessorBean FlowProcessorBean = (FlowProcessorBean) FlowBeanFactory
		.getFlowBean(BeanType.Bean_Type_FlowProcessor);
		return FlowProcessorBean.rejectTask(taskId, userId);
	}
	...
	public boolean revokeTask(String taskId, String userId) throws WFException {
		FlowProcessorBean FlowProcessorBean = (FlowProcessorBean) FlowBeanFactory
		.getFlowBean(BeanType.Bean_Type_FlowProcessor);
		return FlowProcessorBean.revokeTask(taskId, userId);
	}
	...
	public boolean confirmRejectTask(String taskId, String userId)
		...
		FlowProcessorBean FlowProcessorBean = (FlowProcessorBean) FlowBeanFactory
		.getFlowBean(BeanType.Bean_Type_FlowProcessor);
		return FlowProcessorBean.confirmRejectTask(taskId, userId);
	}
	...
	public boolean denyRejectTask(String taskId, String userId)
		...
		FlowProcessorBean FlowProcessorBean = (FlowProcessorBean) FlowBeanFactory
		.getFlowBean(BeanType.Bean_Type_FlowProcessor);
		return FlowProcessorBean.denyRejectTask(taskId, userId);
	}
	...
	public boolean takeTask(String taskId, String userId) throws WFException {
		FlowProcessorBean FlowProcessorBean = (FlowProcessorBean) FlowBeanFactory
		.getFlowBean(BeanType.Bean_Type_FlowProcessor);
		return FlowProcessorBean.takeTask(taskId, userId);
	}
	...
	public boolean returnTask(String taskId, String userId) throws WFException {
		FlowProcessorBean FlowProcessorBean = (FlowProcessorBean) FlowBeanFactory
		.getFlowBean(BeanType.Bean_Type_FlowProcessor);
		return FlowProcessorBean.returnTask(taskId, userId);
	}
	...
	public List queryOwnTasks(String ownerType, String ownerId, String status)
		...
		return FlowProcessorBean.queryOwnTasks(ownerType, ownerId, status);
	}
	...
	public List queryShareTasks(String ownerType, String ownerId, String status)
		...
		FlowProcessorBean FlowProcessorBean = (FlowProcessorBean) FlowBeanFactory
		.getFlowBean(BeanType.Bean_Type_FlowProcessor);
		return FlowProcessorBean.queryShareTasks(ownerType, ownerId, status);
	}
	...
	public boolean changeTaskAssignee(String taskId,String oldUserId,String newUserId) throws WFException{
		FlowProcessorBean FlowProcessorBean = (FlowProcessorBean) FlowBeanFactory
		.getFlowBean(BeanType.Bean_Type_FlowProcessor);
		return FlowProcessorBean.changeTaskAssignee(taskId, oldUserId, newUserId);
	}
