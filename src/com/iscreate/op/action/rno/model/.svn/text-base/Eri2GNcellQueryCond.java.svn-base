 ...
 *
 */
public class Eri2GNcellQueryCond {
	private long cityId;
	...
	public String buildSelectCont() {
		...
		String field = " TO_CHAR(MEA_DATE,'YYYY-MM-DD') MEA_DATE,MSC,t.ENGNAME BSC,CELL,N_BSC,N_CELL,";
		if (!StringUtils.isBlank(param) && !"ALL".equals(param)) {
			//逗号字符串
			field += param;
		}else{
			field+="AWOFFSET    ,BQOFFSET    ,BQOFFSETAFR ,BQOFFSETAWB ,CAND        ,CS          ,GPRSVALID   ,HIHYST      ,KHYST       ,KOFFSET     ,LHYST       ,LOFFSET     ,LOHYST      ,OFFSET      ,PROFFSET    ,TRHYST      ,TROFFSET    ,USERDATA    ,CITY_ID";
		}
		String from=", row_number() OVER(ORDER BY null) AS rn from rno_2g_eri_ncell_param t1  left join(rno_bsc t) on (t1.bsc=t.bsc_id) ";
		select=select+field+from;
		return select;
	}
	public String buildFromCont() {
		String from="  rno_2g_eri_ncell_param t1  left join(rno_bsc t) on (t1.bsc=t.bsc_id) ";
		return from;
	}
	public String buildFieldOutCont() {
		//select MEA_DATE,MSC,t2.ENGNAME BSC,CELL from rno_2g_eri_ncell_param t1  left join(rno_bsc t2) on (t1.bsc=t2.bsc_id) 
		String field_out = " TO_CHAR(MEA_DATE,'YYYY-MM-DD') MEA_DATE,MSC,BSC,CELL,N_BSC,N_CELL,";
		if (!StringUtils.isBlank(param) && !"ALL".equals(param)) {
			//逗号字符串
			field_out += param;
		}else{
			field_out+="AWOFFSET    ,BQOFFSET    ,BQOFFSETAFR ,BQOFFSETAWB ,CAND        ,CS          ,GPRSVALID   ,HIHYST      ,KHYST       ,KOFFSET     ,LHYST       ,LOFFSET     ,LOHYST      ,OFFSET      ,PROFFSET    ,TRHYST      ,TROFFSET    ,USERDATA    ,CITY_ID";
		}
		return field_out;
	}
	public String buildFieldInnerCont() {
		//select MEA_DATE,MSC,t2.ENGNAME BSC,CELL from rno_2g_eri_ncell_param t1  left join(rno_bsc t2) on (t1.bsc=t2.bsc_id) 
		String field_inner = "  MEA_DATE,MSC,t.ENGNAME BSC,CELL,N_BSC,N_CELL,";
		if (!StringUtils.isBlank(param) && !"ALL".equals(param)) {
			//逗号字符串
			field_inner += param;
		}else{
			field_inner+="AWOFFSET    ,BQOFFSET    ,BQOFFSETAFR ,BQOFFSETAWB ,CAND        ,CS          ,GPRSVALID   ,HIHYST      ,KHYST       ,KOFFSET     ,LHYST       ,LOFFSET     ,LOHYST      ,OFFSET      ,PROFFSET    ,TRHYST      ,TROFFSET    ,USERDATA    ,CITY_ID";
		}
		return field_inner;
	}
	public String buildFieldExportCont() {
		String field_out = "MEA_DATE,MSC,BSC,CELL,N_BSC,N_CELL,";
		if (!StringUtils.isBlank(param) && !"ALL".equals(param)) {
			//逗号字符串
			field_out += param;
		}else{
	...
	public static void main(String[] args) {
		Eri2GNcellQueryCond cond=new Eri2GNcellQueryCond();
		cond.setBsc("83147,83148,83149,83150,83151,83152,83153,83154,83155,83156,83157");
		cond.setMeaDate("2014-10-15");
		cond.setCell("ALL");
		cond.setNcell("ALL");
		cond.setCityId(89);
		cond.setParam("CS,GPRSVALID,HIHYST,KHYST,KOFFSET,LHYST,LOFFSET,LOHYST");
