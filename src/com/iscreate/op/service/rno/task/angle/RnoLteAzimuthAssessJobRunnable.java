...
import com.iscreate.op.dao.rno.RnoNcsDynaCoverageHiveDao;
import com.iscreate.op.dao.rno.RnoNcsDynaCoverageHiveDaoImpl;
...
public class RnoLteAzimuthAssessJobRunnable extends BaseJobRunnable {

	private static Log log = LogFactory.getLog(RnoLteAzimuthAssessJobRunnable.class);
	private static String jobType = "RNO_LTE_AZIMUTH_ASSESS";
...
//		JobWorker worker = super.getJobWorker();
		...
		RnoNcsDynaCoverageHiveDao dynaCoverageHiveDao = new RnoNcsDynaCoverageHiveDaoImpl();
...
//			status.setJobState(JobState.Failed);
		...
		// 获取任务的详细信息
		String sql = "";
		sql = "select JOB_ID,BEG_MEA_TIME,END_MEA_TIME,CITY_ID,DL_FILE_NAME,RESULT_DIR,FINISH_STATE,STATUS,CREATE_TIME,MOD_TIME from RNO_LTE_AZIMUTH_ASSESSMENT_JOB where job_id="
				+ jobId;
		...
		// 反射为对象
		RnoLteAzimuthAssessJobRec jobRec = RnoHelper.commonInjection(
				RnoLteAzimuthAssessJobRec.class, rawDatas.get(0),new DateUtil());
		log.debug("LTE方位角评估分析的数据信息：" + jobRec);
...
//		status.setJobRunningStatus(JobRunningStatus.Running);
		...
		sql = "update RNO_LTE_AZIMUTH_ASSESSMENT_JOB set FINISH_STATE='开始分析' where job_id="
				+ jobId;
		try {
		...
		try {
			...
			resultInfo = dynaCoverageHiveDao.doLteAzimuthAssess(this,
...
//				status.setJobRunningStatus(JobRunningStatus.Succeded);
				...
				finishState = DataParseStatus.Failall.toString();
				attMsg = "LTE方位角评估分析异常！" + resultInfo.getMsg();
			}
			report.setFields(stage, begTime, endTime, finishState, attMsg);
			addJobReport(report);
		} catch (Exception e) {
			log.error(jobId + "的LTE方位角评估分析出错！");
			e.printStackTrace();
			endTime = new Date();
			// 状态
			status.setJobState(JobState.Failed);
		...
		log.debug("LTE方位角评估分析任务完成，result=" + resultInfo);
		status.setUpdateTime(new Date());
		
		// 更新结构分析表
		sql = "update RNO_LTE_AZIMUTH_ASSESSMENT_JOB set FINISH_STATE='分析完成' where job_id="
				+ jobId;
		try {
			stmt.executeUpdate(sql);
		} catch (SQLException e) {
	...
	private JobStatus clearWhenStop(JobProfile job, Connection connection,
		...
		try {
			stmt.executeUpdate("update RNO_LTE_AZIMUTH_ASSESSMENT_JOB set FINISH_STATE='被停止' where job_id="
					+ jobId);
		} catch (SQLException e2) {
			e2.printStackTrace();
		}
	...
	public void updateOwnProgress(JobStatus jobStatus) {
		...
		if(stmt!=null){
			try {
				...
				stmt.executeUpdate("update RNO_LTE_AZIMUTH_ASSESSMENT_JOB set FINISH_STATE='"+prog+"',mod_time=to_date('"+new DateUtil().format_yyyyMMddHHmmss(jobStatus.getUpdateTime())+"','yyyy-mm-dd hh24:mi:ss') where job_id="+jobStatus.getJobId());
			} catch (SQLException e) {
				e.printStackTrace();
			}
