...
public class IsAsyncDispatcher extends AbstractService implements Dispatcher {
	...
	public IsAsyncDispatcher() {
		this(new LinkedBlockingQueue<Event>());
	}

	public IsAsyncDispatcher(BlockingQueue<Event> eventQueue) {
		super("IsAsyncDispatcher");
	...
	protected void serviceStart() throws Exception {
		...
		eventHandlingThread.setName("IsAsyncDispatcher event handler");
	...
	protected void serviceStop() throws Exception {
		if (drainEventsOnStop) {
			blockNewEvents = true;
			LOG.info("IsAsyncDispatcher is draining to stop, igonring any new events.");
			synchronized (waitForDrained) {
				while (!drained && eventHandlingThread.isAlive()) {
					waitForDrained.wait(1000);
					LOG.info("Waiting for IsAsyncDispatcher to drain.");
				}
			}
		}
		stopped = true;
	...
	class GenericEventHandler implements EventHandler<Event> {
		public void handle(Event event) {
			...
			try {
				...
				if (!stopped) {
					LOG.warn("IsAsyncDispatcher thread interrupted", e);
				}
